...
func TestBrightSkyCacheAddAndGetRecords(t *testing.T) {
	...
	// Verify last access time was set
	if cache.lastAccess[key].IsZero() {
		t.Error("Last access time was not set")
	}
}
...
func TestBrightSkyCacheCleanupStaleLocations(t *testing.T) {
	cache := &brightSkyCache{
		records:    make(map[string][]brightSkyWeatherRecord),
		date:       make(map[string]string),
		lastAccess: make(map[string]time.Time),
	...
	cache.date[staleKey] = currentDate
	cache.lastAccess[staleKey] = time.Now().Add(-8 * 24 * time.Hour)

	// Add active location (accessed recently)
	activeKey := getCacheKey(40.7128, -74.0060)
	cache.records[activeKey] = []brightSkyWeatherRecord{{Temperature: 10.0}}
	cache.date[activeKey] = currentDate
	cache.lastAccess[activeKey] = time.Now()

	// Trigger cleanup
	cache.mu.Lock()
	cache.cleanupStaleLocations()
	cache.mu.Unlock()

	// Verify stale location was removed
	if _, exists := cache.records[staleKey]; exists {
		t.Error("Stale location should have been removed")
	}

	// Verify active location still exists
	if _, exists := cache.records[activeKey]; !exists {
		t.Error("Active location should not have been removed")
	}
}
